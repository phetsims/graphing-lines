...
define( function( require ) {
  ...
  return inherit( ChallengeFactory, ChallengeFactory2, {
    ...
    createChallenges: function( xRange, yRange ) {
      ...
      {
        slope = RandomChooser.rangeToArray( slopeArrays, slopeArrayIndices ); // first required slope, unique
        yIntercept = RandomChooser.rangeToArray( yInterceptArrays ); // unique y-intercept
      ...
      {
        slope = RandomChooser.rangeToArray( slopeArrays ); // unique slope
        yIntercept = RandomChooser.rangeToArray( yInterceptArrays, yInterceptArrayIndices ); // first required y-intercept, unique
      ...
      {
        slope = RandomChooser.rangeToArray( slopeArrays, slopeArrayIndices );  // second required slope, unique
        yIntercept = RandomChooser.rangeToArray( yInterceptArrays ); // unique y-intercept
      ...
      {
        slope = RandomChooser.rangeToArray( slopeArrays ); // unique slope
        yIntercept = RandomChooser.rangeToArray( yInterceptArrays, yInterceptArrayIndices ); // second required y-intercept, unique
      ...
      // Graph-the-Line, point-slope form, point or slope variable (random choice)
      {
        ...
        if ( manipulationMode === ManipulationMode.SLOPE ) {
          slope = RandomChooser.rangeToArray( slopeArrays, slopeArrayIndices ); // third required slope, unique
